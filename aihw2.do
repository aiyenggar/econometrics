...
reg lhwage yeduc
outreg2 using  `imagepath'Ia-1.tex, ctitle(baseline) tex(pretty frag) dec(4) replace
...
* Question I b 2SLS and Instrument Variables.  Why might the coefficient on an OLS  regression of log wages on years of schooling not be a good estimate of b?  *
...
// State the requirements for the z variables to be good instruments. Explain why the z variables would be good instruments
...
outreg2 using  `imagepath'Ia-1.tex, ctitle(sepa2sls) drop (_*) tex(pretty frag) dec(4) append
xi: ivregress 2sls lhwage (yeduc = z*) i.YOB*ch71
outreg2 using  `imagepath'Ia-1.tex, ctitle(2sls) drop (_*) tex(pretty frag) dec(4) append
estat endog
estat firststage
estat overid
...
* Question II a *
local mainctrl dincome dunemp dmnfcg ddens dwhite dfeml dage65 dhs dcoll ///
	durban dpoverty blt1080 blt2080 bltold80 dplumb vacant70 vacant80 ///
	vacrnt70 downer drevenue dtaxprop depend deduc dwelfr dhlth dhghwy
...
local keepmain dgtsp dincome dunemp dwhite dage65 dpoverty                
...
reg dlhouse dgtsp
outreg2 using  `imagepath'IIa-1.tex, ctitle(baseline) tex(pretty frag) dec(4) replace
...
outreg2 using  `imagepath'IIa-1.tex, ctitle(mainctrl) keep(`keepmain') tex(pretty frag) dec(4) append
...
outreg2 using  `imagepath'IIa-1.tex, ctitle(mainpoly) keep(`keepmain' `keeppoly') tex(pretty frag) dec(4) append
...
outreg2 using  `imagepath'IIa-1.tex, ctitle(baseline) tex(pretty frag) dec(4) append
